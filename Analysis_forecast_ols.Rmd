...
# Fit a OLS model using feols from the fixest package
...
# Apply OLS model for latitude condition less than -10

```{r}
model_data_less_10 <- prepare_model_data(ais_data, c(-Inf,-10))
OLS_model_less_10 <- fit_model(model_data_less_10, "feols")
```


```{r}
# Plot model coefficients
coefplot(OLS_model_less_10)
```

# Apply model for latitude condition greater than -10
```{r}
model_data_greater_10 <- prepare_model_data(ais_data, c(-10,Inf))
OLS_model_greater_10 <- fit_model(model_data_greater_10, "feols")

```

```{r}
# Plot model coefficients
coefplot(OLS_model_greater_10)
```
# Save both models

```{r}
saveRDS(OLS_model_greater_10, "OLS_model_greater_10.rds")
saveRDS(OLS_model_less_10, "OLS_model_less_10.rds")
```
...
# Model prediction for OLS_model_greater_than_10
```{r}
...
model_used_data_greater_10 =
  prepare_model_used_data(OLS_model_greater_10,
                          model_data_greater_10)

combined_results_greater_10 <- 
  process_and_combine_forecasts(OLS_model_greater_10, model_used_data_greater_10, forecast_data_sst, scenarios, time_periods)
...
# Model prediction for OLS_model_less_than_10
```{r}
...
model_used_data_less_10 =
  prepare_model_used_data(OLS_model_less_10,
                          model_data_less_10)

combined_results_less_10 <- 
  process_and_combine_forecasts(OLS_model_less_10, model_used_data_less_10, forecast_data_sst, scenarios, time_periods)
...
```{r}
combined_results_OLS = 
  bind_rows(combined_results_less_10,
            combined_results_greater_10)

#saveRDS(ais_data,"ais_data.rds")

saveRDS(combined_results_OLS, "combined_results_OLS.rds")
```
