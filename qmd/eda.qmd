...
We can fit a simple linear regression to VIIRS detections against fishing effort and look at the residuals. We see that the residuals are generally fairly evenly spaced around 0, although for very low AIS-based effort there are many very high residuals, indicating pixel-months that have much more VIIRS detections than expected.
...
ais_viirs_comparison_data <- ais_viirs_comparison_data |>
  dplyr::mutate(residuals = residuals(model))
