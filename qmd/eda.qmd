...
```{r}
#| label: fig-sst-map
#| fig-cap: Map of mean sea surface temperature (SST) in August 2024, using 0.5x0.5 degree pixels. The currently proposed analysis scope bounding box is shown as a white outline.
...
Here we look at a map of mean sea surface temperature (SST) under 4 different climate change scenarios, for the `Long Term (2081-2100)` forecast horizon (@fig-sst-map-cc-scenarios).

```{r}
#| label: fig-sst-map-cc-scenarios
#| fig-cap: Map of mean sea surface temperature (SST) under 4 different climate change scenarios, for the "Long Term (2081-2100)" forecast horizon. Data are for 1x1 degree pixels. The currently proposed analysis scope bounding box is shown as a white outline.
ggplot() +
  geom_tile(data = sst_cc_forecast_data |>
  dplyr::filter(time_period == "Long Term (2081-2100)"),
            aes(fill = sst_deg_c_mean,
                x = lon_bin,
                y = lat_bin)) +
  geom_sf(data = analysis_bounding_box,
...
Here we look at time series of climate change forecasts for global monthly average SST for each projection time period and scenario (@fig-sst-cc-forecasts-time-series). As expected, projected SST is higher for time periods further into the future, and for more extreme climate change scenarios.

```{r}
#| label: fig-sst-cc-forecasts-time-series
#| fig-cap: Climate change forecasts for global monthly average SST for each projection time period and scenario.
...
#| fig-cap: Map of squid jigger fishing effort from 2016 through August 2024, aggregating effort across effort, flags, and time for each pixel. EEZ boundaries from Marine Regions V12 are shown in orange. The currently proposed analysis scope bounding box is shown as a white outline.
...
```{r}
#| label: fig-viirs-map
#| fig-cap: Map of VIIRS detections, aggregating across time for each pixel. EEZ boundaries from Marine Regions V12 are shown in orange. The currently proposed analysis scope bounding box is shown as a white outline.
...
#| fig-cap: Map showing the distance to the nearest 200nm EEZ/high seas boundary for each 0.25x0.25 degree pixel. 200nm EEZ boundaries from Marine Regions V12 are shown in blank. The currently proposed analysis scope bounding box is shown as a white outline.
ggplot()  +
